...
class MailMessage(models.Model):
    ...
    def _search_is_failed_message(self, operator, value):
        ...
        # Improve performance of subquery on 'mail_tracking_email' table
        # generated by Odoo ORM by restricting the tracking records to fetch
        # to the current messages OR related to the current user
        mail_tracking_operator = "IN" if value else "NOT IN"
        mail_tracking_subquery = f"""
            SELECT mt.mail_message_id
            FROM mail_tracking_email mt
            LEFT JOIN mail_message mm
                ON mt.mail_message_id = mm.id
            WHERE state {mail_tracking_operator} %s
            AND mt.mail_message_id IS NOT NULL
            AND (
                mm.author_id = %s
                OR mt.mail_message_id IN %s
            )
        """
        mail_tracking_args = (
            tuple(self.get_failed_states()),
            self.env.user.partner_id.id,
            tuple(notification_partner_ids.ids),
        )
        self.env.cr.execute(mail_tracking_subquery, mail_tracking_args)
